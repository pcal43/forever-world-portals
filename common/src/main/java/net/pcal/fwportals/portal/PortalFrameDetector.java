...
public final class PortalFrameDetector {
    ...
    public Optional<ForeverWorldPortalFrame> findEmptyFrame(BlockGetter level, BlockPos pos, Direction.Axis preferredAxis, BlockState frameState) {
        return findFrame(level, pos, preferredAxis, frameState, true);
    }

    public Optional<ForeverWorldPortalFrame> findPortalFrame(BlockGetter level, BlockPos pos, Direction.Axis preferredAxis, BlockState frameState) {
        return findFrame(level, pos, preferredAxis, frameState, false);
    }

    public Optional<ForeverWorldPortalFrame> findPortalFrame(BlockGetter level, BlockPos pos, BlockState frameState) {
    ...
    private Optional<ForeverWorldPortalFrame> findFrame(BlockGetter level, BlockPos pos, Direction.Axis preferredAxis, BlockState frameState, boolean requireEmptyInterior) {
        Optional<ForeverWorldPortalFrame> firstAxis = Optional.of(findAnyShape(level, pos, preferredAxis, frameState, requireEmptyInterior))
    ...
    private ForeverWorldPortalFrame findAnyShape(
            BlockGetter level,
            BlockPos pos,
            Direction.Axis axis,
        ...
        BlockPos anchorPos = frameBlocks.stream().min(ForeverWorldPortalFrame.POSITION_COMPARATOR).orElse(bottomLeft);
        return new ForeverWorldPortalFrame(axis, anchorPos, bottomLeft, width, height, List.copyOf(frameBlocks), List.copyOf(interiorBlocks));
    }

    private ForeverWorldPortalFrame emptyFrame(Direction.Axis axis, BlockPos origin) {
        return new ForeverWorldPortalFrame(axis, origin, origin, 0, 0, List.of(), List.of());
    }
    ...
    private List<BlockPos> buildInteriorBlocks(BlockPos bottomLeft, Direction rightDir, int width, int height) {
        ...
        return blocks.stream().sorted(ForeverWorldPortalFrame.POSITION_COMPARATOR).toList();
    }
    ...
    private List<BlockPos> buildFrameBlocks(BlockPos bottomLeft, Direction rightDir, int width, int height) {
        ...
        return blocks.stream().sorted(ForeverWorldPortalFrame.POSITION_COMPARATOR).distinct().toList();
    }
